...
"""
Utility functions for LLM processor configuration to ensure consistency across 
different resource processing components.
"""
...
# Default model to use across all resource processor LLM classes
...
def get_adaptive_model_config() -> Dict[str, Any]:
    ...
    This ensures consistent LLM settings across different processing classes while
    ...
    try:
        ...
        # Adjust batch size based on available memory, but use 1024 as default/medium setting
        if total_memory_gb > 32:
            batch_size = 2048  # High memory machine
        elif total_memory_gb > 4:
            batch_size = 1024  # Default for most machines
        else:
            batch_size = 512   # Very low memory machine
    except ImportError:
        # Default if psutil not available - use 1024 as requested
        batch_size = 1024
    ...
    config = {
        ...
        "batch_size": batch_size,        # Use 1024 as default for most machines
        "num_keep": 64,
        "repeat_penalty": 1.1,
        "temperature": 0.7,
    ...
    logger.info(f"Created adaptive processor config: {recommended_threads} threads, {batch_size} batch size")
    return config
